...
loaded_model = pickle.load(open('xgb_model.sav', 'rb'))
uploaded_file = st.sidebar.file_uploader("Cargue archivo CSV p.e.: {L1: Latitud:0.0, Longitud:0.0}", type=["csv"])
...
if uploaded_file is not None:
    ...
    def user_input_features():
        ...
        city=st.sidebar.selectbox('Seleccione la ciudad Demo',(c.city_ascii))
        n=c[c['city_ascii']==city].index.item()
        lat=c['lat'][n]
        long=c['lng'][n]
        p = st.sidebar.number_input('Numero de Puntos en la Ruta', value=10, min_value=0)#st.sidebar.slider('No de Puntos en la ruta', 5,20,10)

        map_data = pd.DataFrame(
        np.random.randn(p, 2) / [10,10] + [lat, long],
        columns=['lat', 'lon'])
        return map_data

    def iter():
        iter = st.sidebar.number_input('Numero de Iteraciones Optimización', value=10, min_value=0)#st.sidebar.slider('No de Iteraciones Optimización', 10,100,10)
        return iter
...
with app_des:
    ...
    Este trabajo es basado en tbajados hechos por Zach Miller (evolutionary_algorithm_traveling_salesman) y Vladmir Vlazovskiy (route-optimizer-machine-learning)
    """)
...
with row2_1:
    st.write('Localizacion Incial')
    st.pydeck_chart(pdk.Deck(
        map_style="mapbox://styles/mapbox/light-v9",
        initial_view_state={"latitude": dfg.lat.mean(),
                            "longitude": dfg.lon.mean(), "zoom": 10, "pitch": 50},
        layers=[
            pdk.Layer(
            "TextLayer",
            data=dfg,
